...
/**
 * Agento Core Module
 * Documentation class for MagerunTool - Contains all command documentation
 */
...
/**
 * Base documentation class for Magerun MCP Tool.
 * 
 * This class contains comprehensive documentation for 100+ n98-magerun2 commands.
 * The actual implementation is in the MagerunTool class which extends this.
 * 
 ...
 *   - Activate: {"command": "admin:user:activate", "username": "admin"}
 *   - Deactivate: {"command": "admin:user:change-status", "deactivate": true, "username": "admin"}
 *   - List users: {"command": "admin:user:list", "format": "json"}
 *   - Delete user: {"command": "admin:user:delete", "username": "admin", "force": true}
 * 
 * Cache Management:
 *   - List caches: {"command": "cache:list", "format": "json"}
 *   - Clean cache: {"command": "cache:clean", "type": "config"}
 *   - Flush all: {"command": "cache:flush"}
 * 
 * Database Operations:
 *   - Run query: {"command": "db:query", "query": "SELECT * FROM admin_user LIMIT 5"}
 *   - Dump database: {"command": "db:dump", "path": "backup.sql", "force": true}
 *   - Show status: {"command": "db:status", "format": "json"}
 * 
 * Customer Management:
 *   - List customers: {"command": "customer:list", "website": "base"}
 *   - Get info: {"command": "customer:info", "email": "customer@example.com"}
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 * AVAILABLE COMMAND CATEGORIES
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * - admin:*          Admin user management (create, delete, activate, list, tokens)
 * - cache:*          Cache operations (clean, flush, list, enable, disable)
 * - config:*         Configuration management (get, set, delete, search)
 * - customer:*       Customer operations (create, list, info, delete)
 * - db:*             Database operations (query, dump, import, info, status)
 * - eav:*            EAV attribute management (list, view, remove)
 * - dev:*            Development tools (symlinks, assets, template hints, modules)
 * - index:*          Indexer management (list, reindex, status)
 * - integration:*    API integration management (create, list, show)
 * - module:*         Module management (enable, disable, list, status)
 * - route:*          Route inspection (list routes by area/module)
 * - sys:*            System commands (cron, store list, URL list, check)
 * 
 * ═══════════════════════════════════════════════════════════════════════════════
 * TECHNICAL IMPLEMENTATION
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * The implementation overcomes a critical limitation in the php-mcp/server library:
 * - The library extracts JSON-RPC arguments and maps them to method parameters by name
 * - Numeric keys like "0", "1", "2" are filtered out during parameter extraction
 * - PHP cannot have numeric parameter names ($0, $1, $2)
 * 
 * Our solution:
 * 1. Accept descriptive parameters (username, email, query, path, etc.)
 * 2. Auto-map them to positional arguments with priority order
 * 3. Provide generic fallbacks (arg0, arg1, arg2) for edge cases
 * 
 * Example mapping:
 * - Input: {"command": "admin:user:activate", "username": "admin"}
 * - Processed: $username is mapped to position 0
 * - Executed: php magerun admin:user:activate admin
 * 
 * For complete command documentation with all parameters and examples:
 * @see https://github.com/netz98/n98-magerun2 Official magerun documentation
 */
abstract class MagerunToolDoc
{
    /**
     * @var Logger|null
     */
   ...
   * 
   * @param string $command Full magerun command path (e.g., "admin:user:activate", "cache:clean", "db:query")
   ...
   * @param string|null $scope Config scope (default, websites, stores)
   * @param string|null $scopeId Scope ID
   * @param string|null $filename Filename for db:dump, db:import
   * @param string|null $entityType Entity type for EAV commands
   * @param string|null $attributeCode Attribute code for EAV commands
   ...
   * @param string|null $arg3 Fourth positional argument
   * @param string|null $arg4 Fifth positional argument
   * @param string|null $arg5 Sixth positional argument
   * @param bool $activate Activate flag
   * @param bool $deactivate Deactivate flag
   * @param bool $force Force flag (skip confirmations)
   * @param bool $noNewline No newline flag
   * @param bool $enabled Enabled flag
   * @param bool $suppressEvent Suppress event flag
   * @param bool $connection Connection flag
   * @param bool $useMycliInsteadOfMysql Use mycli instead of mysql
   * @param bool $noAutoRehash No auto-rehash flag
   * @param bool $strip Strip flag for db:dump
   * @param bool $gzip Gzip compression flag
   * @param bool $gitFriendly Git-friendly output flag
   * @param bool $humanReadable Human-readable format flag
   * @param bool $noViews No views flag
   * @param bool $drop Drop database before import
   * @param bool $dropTables Drop tables before import
   * @param bool $onlyIfEmpty Only if empty flag
   * @param bool $optimize Optimize flag
   * @param bool $skipAuthorizationEntryCreation Skip authorization entry creation
   * @param bool $rounding Rounding flag
   ...
   * @param bool $addSource Add source flag
   * @param string|null $group Group parameter
   * @param string|null $module Module parameter
   * @param string|null $key Configuration key
   * @param string|null $value Configuration value
   * @param bool $dd Debug dump - when true, dumps all received parameters and exits
   * @return TextContent
   */
    abstract public function executeMagerun(
        string $command = '',
